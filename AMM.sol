...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		if( sellToken == tokenA ) {
			uint256 balanceBefore = qtyA;
			ERC20(tokenA).transferFrom(msg.sender, address(this), sellAmount);
			uint256 balanceAfter = ERC20(tokenA).balanceOf(address(this));
			uint256 actualIn = balanceAfter - balanceBefore;
			require( actualIn > 0, 'No tokens received' );
			uint256 amountInWithFee = actualIn * (10000 - feebps);
			swapAmt = (amountInWithFee * qtyB) / (balanceBefore * 10000 + amountInWithFee);
			require( swapAmt > 0, 'Insufficient output amount' );
			ERC20(tokenB).transfer(msg.sender, swapAmt);
			emit Swap( tokenA, tokenB, actualIn, swapAmt );
		} else {
			uint256 balanceBefore = qtyB;
			ERC20(tokenB).transferFrom(msg.sender, address(this), sellAmount);
			uint256 balanceAfter = ERC20(tokenB).balanceOf(address(this));
			uint256 actualIn = balanceAfter - balanceBefore;
			require( actualIn > 0, 'No tokens received' );
			uint256 amountInWithFee = actualIn * (10000 - feebps);
			swapAmt = (amountInWithFee * qtyA) / (balanceBefore * 10000 + amountInWithFee);
			require( swapAmt > 0, 'Insufficient output amount' );
			ERC20(tokenA).transfer(msg.sender, swapAmt);
			emit Swap( tokenB, tokenA, actualIn, swapAmt );
		}
